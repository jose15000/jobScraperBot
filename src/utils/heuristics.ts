...
export function runJobHeuristic(content: string): FilterResult {
  ...
  if (seniorMatches.length > 0 && juniorMatches.length === 0) {
    return {
      ...
      dealbreakersFound,
      reason:
        'Vaga destinada apenas para níveis superiores (Pleno/Sênior/Lead)',
    };
  }
  ...
  const shouldApply =
    hiringMatches.length > 0 && techMatches.length > 0 && score >= 25;
  ...
  if (!shouldApply) {
    if (hiringMatches.length === 0) reason = 'Sem termos claros de contratação';
      ...
      reason = 'Nenhuma tecnologia corresponde à stack';
